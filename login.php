...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($parola)) {
        ...
        $stmt = $conn->prepare("SELECT id, nume, parola, COALESCE(rol,'client') AS rol FROM utilizatori WHERE email = ?");
        $stmt->bind_param("s", $email);
        $stmt->execute();
        $stmt->store_result();

        if ($stmt->num_rows > 0) {
            $stmt->bind_result($id, $nume, $parolaHash, $rol);
            $stmt->fetch();
...
<head>
    ...
    <title>Autentificare - Carp Travel</title>
